...
class JDBook():
    ...
    def access_to_search_page(self):
        '''通过登录进入检索结果网页'''
        input_search = self.driver.find_element(By.CSS_SELECTOR,value='#key')
        time.sleep(2)
        input_search.send_keys('java')
        search_button = self.driver.find_element(By.CSS_SELECTOR,value='#search-2014 > div > button')
        time.sleep(2)
        search_button.click()
        # 之后手动扫码进入了检索结果页
    ...
    def get_data(self):
        ...
        self.driver.execute_script('window.scrollTo(0,document.body.scrollHeight)')
         # 通过execute_script来执行js代码，
         # 0是起始位置，document.body.scrollHeight是整个窗口的高度
        li_tags = self.driver.find_elements(By.CSS_SELECTOR,'#J_goodsList > ul > li')
    ...
    def write_to_csv(self,data,index_header):
        '''将解析得到的数据写入csv文件'''
    ...
    def main(self):
        self.driver.get('https://book.jd.com/')
        time.sleep(2)
        self.access_to_search_page()
        i = 1
        if self.driver.find_element(By.CSS_SELECTOR,value='#key'):
