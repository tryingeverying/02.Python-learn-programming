...
def get_one_page(url):
    try:
        ...
        if response.status_code == 200:
            ...
            titleElems = res('#content > div > div.article > ol > li > div > div.info > div.hd > a > span:nth-child(1)')
            return titleElems
    except RequestException:
        return None

def write_to_file(content):
    with open(r'F:\Programming\02.Python-learn-programming\6_ Python3_Web_Crawler_Development_Practice\Chapter_4_Using_the_Parsing_Library\douban_ressult.txt', 'a', encoding='utf-8', ) as f:
        f.write(content+'\n')

def main():
    for i in range(0,100,25):
        url = 'https://movie.douban.com/top250?start='+str(i)+'&filter='
        title = get_one_page(url)
        time.sleep(0.5)
